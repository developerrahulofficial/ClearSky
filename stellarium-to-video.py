...
class StellariumToVideo:
    ...
    def create_script(self, script_path: Path) -> None:
        ...
        script = script.replace("$DATE$",
                                self.__param.start_date.astimezone(timezone.utc).isoformat().replace("+00:00", ""))
    ...
    def create_frames(self) -> None:
        proc_stellarium = subprocess.Popen(
            ['stellarium', '--startup-script', 'stellarium_to_video.ssc', '--screenshot-dir', str(self.__frame_folder)],
    ...
    def create_video(self) -> None:
        ...
        print(
            f"ffmpeg -y -r {str(self.__param.fps)} -f image2 -i f'{self.__frame_folder}/frame_%03d.png' -s {self.__param.video_size} -crf 12 -pix_fmt yuv420p")
...
def check_prerequisites(param: Parameters) -> Path:
    ...
    if stellarium_path is None:
        raise Exception(
            'Stellarium not found! This script requires Stellarium to be installed and available in the system path!')
    else:
    ...
    if ffmpeg_path is None:
        raise Exception(
            'FFMPEG not found! This script requires FFMPEG to be installed and available in the system path!')
    else:
    ...
    if not os.path.isdir(stellarium_data_path.absolute()):
        raise Exception(
            f'Cannot find the Stellarium user data path ({stellarium_data_path.absolute()}). Is Stellarium properly installed?')
    ...
    if not os.path.isdir(script_folder.absolute()):
        raise Exception(
            f'The folder for stellarium user scripts cannot be found and an attempt to create it failed! ({script_folder})')

    if not param.template_file.exists():
        raise Exception(
            f'The template script "{param.template_file}" does not exist in the script folder "{script_folder}"')
...
def main() -> None:
    ...
    parser.add_argument("-c", "--Caption", dest="caption", help='Caption of the video', required=False,
                        default='The Night Sky')
    parser.add_argument("-d", "--DateTime", dest="date",
                        help='A date time string in UTC. If no date is given todays date is used. If no time is given the animation automatically starts an hour after sunset.',
                        required=False, type=arg_to_start_date, default=date.today().isoformat())
    parser.add_argument("-dt", "--DeltaT", dest="dt",
                        help='Simulated time in between two Frames as an ISO 8601 duration (default="PT20S" -> 20 seconds)',
                        default='PT20S', type=arg_to_iso_8661_duration)
    parser.add_argument("-fps", "--FramesPerSecond", dest="fps", help='Frame rate of the output video', default='30',
                        type=arg_to_positive_number)
    parser.add_argument("-l", "--Location", dest="loc", help='Location of the observer', default="13.9,50.9",
                        required=True, type=arg_to_location)
    ...
    parser.add_argument("-s", "--Show", dest="show_video", default=False, action='store_true',
                        help='If this flag is set the video is shown after rendering (VLC must be installed)')
    parser.add_argument("-t", "--Template", dest="template",
                        help='The template script. This must be the name of a ssc script in the script folder)',
                        required=False, default='default.ssc', type=str)
    parser.add_argument("-ts", "--TimeSpan", dest="timespan",
                        help='Total time span covered by the simulation as ISO 8601 duration (default="PT2H" -> 2 hours)',
                        default='PT2H', type=arg_to_iso_8661_duration)
    parser.add_argument("-v", "--View", dest="view", help='Defines the view Altitude, Azimuth, Field of View',
                        default="180,35.0,70.0", type=arg_to_vec3)
    parser.add_argument("-sz", "--VideoSize", dest="video_size",
                        help='The size of the output video. Example: "1920x1080"', default="1920x1080", required=False,
                        type=arg_to_size)
    parser.add_argument("-wsz", "--WindowSize", dest="window_size",
                        help='The size of the stellarium window. Example: "1080x1920" for portrait mode', default=None,
                        required=False, type=arg_to_size)
    ...
    if param.start_at_sunset:
        print(
            '\033[93m' + 'Warning: No time given. The script will try to use the sunset time of the given day!' + '\033[0m')
